...
function countVowels(str) {
  return str.match(/[aeiouyAEIOUY]/g) ? str.match(/[aeiouyAEIOUY]/g).length : 0;
}
...
function findLongestWord(sentence) {
  ...
  return words.reduce((longestWord, currentWord) => {
    ...
    return cleanWord.length > longestWord.length ? cleanWord : longestWord;
  }, '');
}
...
function extractNameFromTemplate(value) {
  return value
    .substring(value.indexOf('Hello') + 'Hello, '.length, value.indexOf('!'))
    .trim();
}
